...
public class Vibration implements ItestSuite {
    @Override
    public Constants.Result execute(AppiumDriver<MobileElement> driver, int expectedOutput) {
        System.out.println("Running..... Vibration Test");
        driver.executeScript("mobile: shell", UtilCommon.commandGenerator(Constants.VIBRATION));

        if (expectedOutput == 2) {
            try {
                TestSuite.captureScreenshot(Constants.VIBRATION);
