...
public class Speaker implements ItestSuite {
    @Override
    public Constants.Result execute(AppiumDriver<MobileElement> driver, int expectedOutput) {
        System.out.println("Running..... Speaker Test");
        driver.executeScript("mobile: shell", UtilCommon.commandGenerator(Constants.SPEAKER));

        if (expectedOutput == 2) {
            try {
                TestSuite.captureScreenshot(Constants.SPEAKER);
