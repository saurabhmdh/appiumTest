...
public class CamRearVideo implements ItestSuite {
    @Override
    public Constants.Result execute(AppiumDriver<MobileElement> driver, int expectedOutput) {
        System.out.println("Running..... Rear Camera Video Test");
        driver.executeScript("mobile: shell", UtilCommon.commandGenerator(Constants.REAR_CAMERA_VIDEO));

        if (expectedOutput == 2) {
            try {
                TestSuite.captureScreenshot(Constants.REAR_CAMERA_VIDEO);
    ...
    public String getOriginalName() {
        return Constants.REAR_CAMERA_VIDEO;
    }
