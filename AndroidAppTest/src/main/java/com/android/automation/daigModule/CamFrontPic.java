...
public class CamFrontPic implements ItestSuite {

    @Override
    public Constants.Result execute(AppiumDriver<MobileElement> driver, int expectedOutput) {
        System.out.println("Running..... Front Camera Test");
        driver.executeScript("mobile: shell", UtilCommon.commandGenerator(Constants.FRONT_CAMERA_PICTURE));

        if (expectedOutput == 2) {
            try {
                TestSuite.captureScreenshot(Constants.FRONT_CAMERA_PICTURE);
    ...
    public String getOriginalName() {
        return Constants.FRONT_CAMERA_PICTURE;
    }
