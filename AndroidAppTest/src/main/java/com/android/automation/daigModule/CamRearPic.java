...
public class CamRearPic implements ItestSuite {
    @Override
    public Constants.Result execute(AppiumDriver<MobileElement> driver, int expectedOutput) {
        System.out.println("Running..... Rear Cam Pic Test");
        driver.executeScript("mobile: shell", UtilCommon.commandGenerator(Constants.REAR_CAMERA_PICTURE));

        if (expectedOutput == 2) {
            try {
                TestSuite.captureScreenshot(Constants.REAR_CAMERA_PICTURE);
    ...
    public String getOriginalName() {
        return Constants.REAR_CAMERA_PICTURE;
    }
}
